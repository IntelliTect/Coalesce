...
- Added `ClientCacheDurationSeconds` property to `ExecuteAttribute` to customize client-side cache duration for GET custom methods with `VaryByProperty`. Allows fine-grained control over caching behavior, from disabling cache (0 seconds) to custom durations, while maintaining the 30-day default.
