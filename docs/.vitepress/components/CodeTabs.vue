<template>
  <div class="code-tabs">
    <div v-if="Object.keys(actualLanguages).length > 1" class="code-tabs__nav">
      <ul class="code-tabs__ul">
        <li
          v-for="(langName, shorthand) in actualLanguages"
          :key="shorthand"
          class="code-tabs__li"
        >
          <button
            class="code-tabs__nav-tab"
            :class="{
              'code-tabs__nav-tab-active': selectedLanguage === shorthand,
            }"
...
const options = {
  groups: {
    default: { vue: "Vue" },
    vue: { setup: "Composition API", options: "Options API" },
...
export default {
  ...
  created() {
    ...
    if (typeof localStorage !== "undefined") {
      const selected = localStorage.getItem(this.localStorageKey);
      if (
        selected &&
