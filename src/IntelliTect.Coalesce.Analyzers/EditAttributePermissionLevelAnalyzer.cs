...
namespace IntelliTect.Coalesce.Analyzers
{
    ...
    public class EditAttributePermissionLevelAnalyzer : DiagnosticAnalyzer
    {
        public static readonly DiagnosticDescriptor Rule = new DiagnosticDescriptor(
            "COA002",
            "EditAttribute on a property cannot declare a PermissionLevel",
            "EditAttribute on property '{0}' cannot declare a PermissionLevel. PermissionLevel is only allowed on class-level EditAttribute.",
            "Coalesce",
            DiagnosticSeverity.Error,
            isEnabledByDefault: true,
            description: "EditAttribute on properties should not specify a PermissionLevel parameter. Use roles instead or apply the EditAttribute at the class level.");
        ...
        private static void AnalyzeAttributeList(SyntaxNodeAnalysisContext context)
        {
            ...
            foreach (var attribute in attributeList.Attributes)
            {
                ...
                // Check if this is EditAttribute
                if (!IsEditAttribute(attributeType))
                    continue;
        ...
        private static bool IsEditAttribute(INamedTypeSymbol attributeType)
        {
            return attributeType.Name == "EditAttribute" && 
