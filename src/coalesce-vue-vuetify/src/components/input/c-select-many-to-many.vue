<template>
  <v-autocomplete
    ...
    @input="onInput"
    multiple
    chips :deletable-chips="canDelete" small-chips
...
export default class CSelectManyToMany extends MetadataComponent { 
  ...
  get canDelete() {
    return ((this.collectionMeta.itemType as ModelValue).typeDef.behaviorFlags & BehaviorFlags.Delete) != 0
  }
  ...
  get collectionMeta() {
    ...
    if (valueMeta && valueMeta.type == "collection") {
      ...
      throw Error("c-select-many-to-many requires value metadata for a collection. Specify it with the 'for' prop'");
    }
  }
  ...
  get manyToManyMeta() {
    ...
    throw Error("c-select-many-to-many requires value metadata for a many-to-many collection. Specify it with the 'for' prop'");
  }
