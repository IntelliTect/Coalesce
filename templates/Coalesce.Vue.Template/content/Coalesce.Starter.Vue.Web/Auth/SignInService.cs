...
)
{
    ...
    public async Task OnGoogleTicketReceived(TicketReceivedContext ctx)
    {
        ...
        {
            await GetAndAssignUserExternalTenant(user, remoteLoginInfo, gSuiteDomain);
        }
#endif
    ...
    public async Task OnMicrosoftTicketReceived(TicketReceivedContext ctx)
    {
        ...
        try
        {
            ...
            {
                await GetAndAssignUserExternalTenant(user, remoteLoginInfo, entraTenantId);
            }
        }
        ...
        if (user is null)
        {
            ...
            if (!db.Users.Any())
            {
                ...
                if (tenant is not null) 
                {
                    ...
                    logger.LogInformation($"Granting demo tenant membership for initial user {user.Id}");
                }
#endif
            }
#else
