...
# NOTE: If this is failing to provision due to permission errors, see `terraform/README.md`.
...
resource "time_rotating" "secret_rotation_1" {
  ...
  rotation_months = local.secret_duration_months / 2
}

resource "time_rotating" "secret_rotation_2" {
  rfc3339         = time_rotating.secret_rotation_1.rotation_rfc3339
  rotation_months = local.secret_duration_months / 2 # additive to rfc3339
...
resource "azuread_application_password" "app_secret_1" {
  ...
  end_date       = timeadd(time_rotating.secret_rotation_1.id, "${local.secret_duration_hours}h")
...
resource "azuread_application_password" "app_secret_2" {
  ...
  end_date       = timeadd(time_rotating.secret_rotation_2.id, "${local.secret_duration_hours}h")
